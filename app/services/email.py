...
def _build_patient_email(report: dict[str, Any], filename: str, app_url: str, user_name: str | None = None) -> str:
    ...
    greeting = f'<p style="margin:0 0 18px;font-size:16px;font-weight:600;color:{_EC_TEXT};font-family:\'Segoe UI\',Arial,Helvetica,sans-serif;">Dear {user_name},</p>' if user_name else ''
...
def _build_clinician_email(report: dict[str, Any], filename: str, app_url: str, user_name: str | None = None) -> str:
    result = report.get("result_label", "Unknown")
    risk = report.get("risk_level", "Unknown")
    event_count = report.get("event_count", 0) or 0
    confidence = report.get("confidence_score")
    ...
    greeting = f'<p style="margin:0 0 18px;font-size:16px;font-weight:600;color:{_EC_TEXT};font-family:\'Segoe UI\',Arial,Helvetica,sans-serif;">Dear Dr. {user_name},</p>' if user_name else ''
    return (
...
def _build_researcher_email(report: dict[str, Any], filename: str, app_url: str, user_name: str | None = None) -> str:
    result = report.get("result_label", "Unknown")
    risk = report.get("risk_level", "Unknown")
    event_count = report.get("event_count", 0) or 0
    confidence = report.get("confidence_score")
...
def _build_email_html(report: dict[str, Any], filename: str, role: str | None, app_url: str, user_name: str | None = None) -> str:
    role_lower = (role or "").lower()
    if role_lower == "patient":
      return _build_patient_email(report, filename, app_url, user_name)
    if role_lower == "researcher":
      return _build_researcher_email(report, filename, app_url, user_name)
    return _build_clinician_email(report, filename, app_url, user_name)
...
def send_report_email_relay(
    *,
    ...
    html = _build_email_html(report, filename, role, _normalize_app_url(app_url), user_name)
